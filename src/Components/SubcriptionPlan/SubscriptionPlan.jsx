...
import { BiSolidCheckCircle, BiXCircle, BiX  } from "react-icons/bi";
...
const SubscriptionPlan = () => {
    ...
    const [payuSign, setPayuSign] = useState("");
    ...
    const handlePaymentX = async (e) => {

        e.preventDefault();

        let referenceCode = `${user?.id}-${priceData?.id}${aid}`;

        let PayuSign = `${process.env.NEXT_PUBLIC_APIKEY_PAYU}~${process.env.NEXT_PUBLIC_MERCHANTID}~${referenceCode}~${priceData?.price}~COP`;
        
        setReferenceCode(referenceCode);
        setPayuSign(md5(PayuSign));

        setLoading(true);

        setTimeout(() => {
          setLoading(false);
          document.querySelector('#payuForm').submit();
        }, "5000");
        ...
        <Layout>
                                ...
                                <>
                                    {sortedPackageData.length > 0 ? (
                                        sortedPackageData.map((elem, index) => (
                                            <SwiperSlide key={index}>
                                                <div className="card text-white" style={{height: "auto !important;"}} id={`${elem.is_active ? "current_package_card" : "other_package_card"}`}>
...

            {
                ...
                <>
                    ...
                    <Modal centered open={showPayuModal} footer={null} onCancel={closePayuModal}>
                        ...
                        <form method="post" action="https://checkout.payulatam.com/ppp-web-gateway-payu/" id="payuForm">
                            <Input name="merchantId"      type="hidden"  value={process.env.NEXT_PUBLIC_MERCHANTID} />
                            <Input name="accountId"       type="hidden"  value={process.env.NEXT_PUBLIC_ACCOUNTID} />
                            <Input name="description"     type="hidden"  value="Pago Suscripción" />
                            <Input name="referenceCode"   type="hidden"  value= {referenceCode} />
                            <Input name="amount"          type="hidden"  value= {priceData?.price} />
